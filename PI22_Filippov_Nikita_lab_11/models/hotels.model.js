...
class Hotels {
    ...
    delete(key, result) {
        sql.query(`UPDATE Hotels SET isDeleted = true WHERE ID = '${key}' RETURNING Countrieid;`, (err, res) => {
            if (err) {
                console.log("Error: ", err);
                result(err, null);
                return;
            ...
            result(null, {message:'Удалено успешно', CountrieID: res.rows[0].countrieid});
        })
    }
}
