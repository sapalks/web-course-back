...
import { error } from './controllers/utils';
...
function commit(request: Request, response: Response): void {
    ...
    const msg = `[${started.toUTCString()}, ${duration}ms] ${reqInfo}, ${response.statusCode}`;
    logger.info(msg);
}

export function accessLog(request: Request, response: Response, next: NextFunction): void {
