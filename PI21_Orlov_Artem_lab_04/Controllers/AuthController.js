...
class AuthController {
    async register(req, res) {
        ...
        await db.query(`INSERT INTO client(login,password) VALUES ($1,$2)`, [login, password])
        res.status(200).json({ status: 'ok', message: 'Client created' })
    }

    async login(req, res) {
        const { login, password } = req.body
        const candidate = await db.query(`SELECT * FROM client WHERE login = $1 AND password = $2`, [login, password])
        if (candidate.rows[0]) {
