...
class DesireController{
    ...
    async getAllDesires(req,res) {
        ...
        const Desires = await db.query('SELECT Name_desire, Degree_of_desire, Timofey_id From Desire')
        res.json(Desires.rows)
    }
    async getDesire(req, res) {
        const id = req.params.id;
        const Desire = await db.query('SELECT Name_desire, Degree_of_desire, Timofey_id From Desire WHERE id = $1', [id])
        res.json(Desire.rows[0])
    }

    async getTimofeysDesire(req, res) {
        const id = req.query.Timofey_id;
        const result = await db.query('SELECT Name_desire, Degree_of_desire, Timofey_id FROM Desire WHERE Timofey_id = $1', [id]);
